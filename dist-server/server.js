...
const admin = adminNamespace.default || adminNamespace;
...
// --- Storage Helpers ---
async function getScheduleConfig(productId) {
    if (db) {
...
async function setScheduleConfig(productId, config) {
    if (db) {
...
async function getPostQueue(productId) {
    if (db) {
...
async function removeFromQueue(productId, postId) {
    if (db) {
...
async function getToken(productId, platform) {
    if (db) {
        const doc = await db.collection('server_tokens').doc(productId).get();
        return doc.exists ? doc.data()?.[platform] : null;
    }
    if (platform === 'linkedin')
        return globalLinkedinTokens[productId];
    if (platform === 'facebook')
        return globalFacebookTokens[productId];
    if (platform === 'instagram')
        return globalInstagramTokens[productId];
    if (platform === 'reddit')
        return globalRedditTokens[productId];
    return null;
}
async function setToken(productId, platform, token) {
    if (db) {
        await db.collection('server_tokens').doc(productId).set({ [platform]: token }, { merge: true });
    }
    else {
        if (platform === 'linkedin')
            globalLinkedinTokens[productId] = token;
        if (platform === 'facebook')
            globalFacebookTokens[productId] = token;
        if (platform === 'instagram')
            globalInstagramTokens[productId] = token;
        if (platform === 'reddit')
            globalRedditTokens[productId] = token;
    }
}
// --- Scheduling State (Fallback) ---
let globalLinkedinTokens = {};
let globalFacebookTokens = {};
let globalInstagramTokens = {};
let globalRedditTokens = {};
let scheduleConfigs = {};
let postQueue = [];
let lastPostedDates = {};
...
async function startServer() {
    ...
    app.get('/api/facebook/status', requireAuth, async (req, res) => {
        try {
            ...
            if (cookieToken)
                await setToken(productId, 'facebook', cookieToken);
    ...
    app.get('/api/instagram/status', requireAuth, async (req, res) => {
        try {
            ...
            if (cookieToken)
                await setToken(productId, 'instagram', cookieToken);
    ...
    app.get('/api/reddit/status', requireAuth, async (req, res) => {
        try {
            ...
            if (cookieToken)
                await setToken(productId, 'reddit', cookieToken);
