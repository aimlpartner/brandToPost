...
if (!content.includes('previewCreative && (')) {
  // Let's replace the final `  </div>\n  );\n}\n` with our injected code + the final closure

  const replacement = `      {/* Full-screen Image Preview Overlay */}
      {previewCreative && (
        <div 
