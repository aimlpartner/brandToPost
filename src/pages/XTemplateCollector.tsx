...
import { ArrowLeft, Sparkles, RefreshCw, Layout, Eye, Copy, Check, Palette, Type, SlidersHorizontal, Flame, AlertCircle, CheckCircle2, Twitter } from 'lucide-react';
...
function renderDiscoveredHtml(rawHtml: string, data: {
  ...
  html = html.replace(/\{\{PRIMARY_COLOR\}\}/g, escapeHtmlAttr(data.primaryColor || "#1d9bf0"));
...
export function XTemplateCollector() {
  ...
  const [primaryColor, setPrimaryColor] = useState<string>('#1d9bf0');
  ...
  // Load Saved X Templates on Mount
  const loadSavedTemplates = async () => {
    try {
      const res = await fetch('/api/research-channel-templates?channel=x');
      if (res.ok) {
        const data = await res.json();
        if (data.success && Array.isArray(data.templates) && data.templates.length > 0) {
          setTemplates(data.templates);
          setSummary(`Loaded ${data.templates.length} saved X templates from disk storage.`);
        }
      }
    } catch (e) {}
  };
  ...
  const handleFetchXTemplates = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch('/api/research-channel-templates', {
        ...
        body: JSON.stringify({
          channel: 'x',
          count: 20,
          niche: 'High-Impact X B2B & Organic Brand Carousels (Micro-Copy, Editorial Hero, Before vs After Split)'
        })
      });
      ...
      if (!data.success || !Array.isArray(data.templates)) {
        throw new Error(data.error || 'Failed to parse discovered X templates.');
      }

      setSummary(data.summary || `Extracted ${data.templates.length} X templates.`);
      setTemplates(data.templates);
    } catch (err: any) {
      setError(err.message || 'An error occurred during X template discovery.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              ...
              X (Twitter) Live Visual Template Collector
            </h1>
            <p className="text-xs text-slate-500">Grounded Research + Raw HTML/CSS Template Synthesis for X Posts</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <button
            onClick={handleFetchXTemplates}
            disabled={loading}
            className={`px-5 py-2 text-xs font-bold rounded-lg border transition-all flex items-center gap-2 ${
              loading
                ? 'bg-slate-100 border-slate-300 text-slate-400 cursor-not-allowed'
                : 'bg-sky-600 hover:bg-sky-500 text-white border-sky-500 shadow-sm hover:shadow-sky-200'
            }`}
          >
            <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} />
            {loading ? 'Collecting 20 X Templates...' : 'Discover & Collect All 20 Live X Templates'}
          </button>
        </div>
      </header>
            ...
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-slate-50 border border-slate-200 rounded p-2 text-xs text-slate-900 focus:bg-white focus:border-sky-500 focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-slate-50 border border-slate-200 rounded p-2 text-xs text-slate-700 focus:bg-white focus:border-sky-500 focus:outline-none"
                />
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <label className="text-[9px] text-sky-600 flex items-center gap-1 cursor-pointer font-bold">
                    <input 
                      type="checkbox" 
                      checked={isTextAuto} 
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded p-2 text-xs text-slate-800 focus:bg-white focus:border-sky-500 focus:outline-none"
                >
                  {FONTS.map((f) => (
                  ...
                  {SAMPLE_LOGOS.map((lg) => (
                    <button
                      ...
                      className={`p-1.5 rounded border text-[10px] truncate transition ${
                        logoUrl === lg.url ? 'border-sky-600 bg-sky-50 text-sky-900 font-bold' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      {lg.name}
                    </button>
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {SAMPLE_IMAGES.map((img) => (
                    <button
                      ...
                      className={`p-1.5 rounded border text-[10px] truncate transition ${
                        imageUrl === img.url ? 'border-sky-600 bg-sky-50 text-sky-900 font-bold' : 'border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      {img.name}
                    </button>
        ...
        <main className="flex-1 p-6 overflow-y-auto space-y-6 bg-slate-50">
          ...
          {summary && (
            <div className="bg-white border border-slate-200 rounded-lg p-4 text-xs space-y-1 shadow-sm">
              <div className="font-bold text-sky-600 flex items-center gap-2">
                <Flame className="w-4 h-4 text-amber-500" /> Grounded X Research Summary
              </div>
          ...
          {templates.length === 0 && !loading && (
            <div className="flex flex-col items-center justify-center p-20 border border-dashed border-slate-300 bg-white rounded-xl space-y-4 text-center shadow-sm">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900">No X Templates Discovered Yet</h3>
                <p className="text-xs text-slate-500 mt-1 max-w-sm">
                  Click "Discover & Collect All 20 Live X Templates" above to perform a live search and synthesize 20 X HTML/CSS templates.
                </p>
              </div>
            </div>
          )}
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center p-20 space-y-3 text-slate-500 text-xs font-mono">
              ...
              <span>Researching live X B2B post trends & synthesizing 20 HTML/CSS templates...</span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {templates.map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className="bg-white border border-slate-200 hover:border-sky-300 rounded-xl p-4 transition flex flex-col space-y-3 shadow-sm hover:shadow-md group"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="text-xs font-bold text-slate-900 leading-tight">{t.name}</h4>
                      <span className="text-[10px] text-sky-600 font-mono block mt-0.5">{t.archetype}</span>
                    </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded bg-sky-600 hover:bg-sky-500 text-white text-[11px] font-bold flex items-center gap-1.5 shadow-sm"
                    >
          ...
          <div className="bg-white border border-slate-200 rounded-xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <span className="text-xs text-sky-600 font-mono">{selectedTemplate.id}</span>
              </div>
              <button
              ...
              <div className="w-full space-y-3 text-xs font-mono text-slate-700">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="px-3 py-1 bg-sky-600 text-white rounded font-sans text-xs font-bold shadow-sm"
                  >
                    Copy HTML
                  </button>
                </div>
...
export default XTemplateCollector;
