      ...
      {postFeedbacks.length > 0 && (
        <div className="space-y-3 mb-4">
          {postFeedbacks.map(feedback => (
            <div key={feedback.id} className="bg-white/60 rounded-lg p-3 text-sm border border-gray-100">
      ...
      {!timerExpired ? (
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            className="w-full bg-white/80 border border-gray-200 rounded-lg pl-3 pr-10 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#ff6347]/50 resize-none min-h-[60px]"
            disabled={isSubmitting}
          />
          <button
            type="submit"
            disabled={!newFeedback.trim() || isSubmitting}
            className="absolute right-2 bottom-2 p-1.5 text-gray-400 hover:text-[#ff6347] disabled:opacity-50 transition-colors"
          >
            <Send className="h-4 w-4" />
          </button>
        </form>
      ) : (
        <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-500 text-center italic border border-gray-100">
